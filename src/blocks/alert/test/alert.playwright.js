...
test.beforeEach( async ( { page, browserName }, info ) => {
	...
	await page.evaluate( `wp.data.dispatch( 'core/editor' ).insertBlocks( wp.blocks.createBlock( '${ blockName }' ) )` );
	await page.evaluate( `wp.data.dispatch( 'core/editor' ).savePost()` );
} );

test( 'Alert block saves with no title or text', async ( { page } ) => {
	await expect( page.locator( `[data-type="${ blockName }"]` ) ).toBeVisible();
	await page.reload();
	await expect( page.locator( `[data-type="${ blockName }"]` ) ).toBeVisible();
} );

test( 'Alert block saves with custom title and text', async ( { page } ) => {
