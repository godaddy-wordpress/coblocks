...
describe( 'Test CoBlocks Form Block', function() {
	/**
	 * Test the coblocks form radio field transforms.
	 */
	it( 'should transform from coblocks/field-radio block to coblocks/field-select, coblocks/field-checkbox', function() {
		...
		cy.get( '[data-type="coblocks/field-radio"]' )
			.click( { force: true } );
		...
		for ( var index = 0; index < blocksToTransformTo.length; index++ ) {
			...
			// Switch the radio field to another type.
			cy.get( '[data-type="coblocks/field-radio"]' )
				.click( { force: true } );
			...
			// Check the radio field transformed properly.
