			...
			{ 'select' !== type && (
				<InspectorControls>
					<PanelBody title={ __( 'Display Settings', 'coblocks' ) }>
						<ToggleControl
							label={ sprintf( /* translators: field type eg: checkbox */ __( 'Inline %s', 'coblocks' ), type ) }
							className="coblocks-field-label__required"
							checked={ isInline }
							onChange={ value => setAttributes( { isInline: value } ) }
							help={ !! isInline ? sprintf( /* translators: field type eg: checkbox */ __( 'Displaying the %s inline.', 'coblocks' ), type ) : sprintf( /* translators: field type eg: checkbox */ __( 'Toggle to display the %s inline.', 'coblocks' ), type ) }
						/>
					</PanelBody>
				</InspectorControls>
			) }
