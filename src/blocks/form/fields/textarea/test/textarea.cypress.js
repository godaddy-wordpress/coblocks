...
describe( 'Test CoBlocks Form Block', function() {
	/**
	 * Test the coblocks form textarea field transforms.
	 */
	it( 'should transform from coblocks/field-textarea block to coblocks/field-date, coblocks/field-name, coblocks/field-phone, coblocks/field-text, coblocks/field-website, coblocks/field-hidden', function() {
		...
		for ( var index = 0; index < blocksToTransformTo.length; index++ ) {
			...
			// Switch the textarea field to another type.
			...
			// Check the textarea field transformed properly.
			...
			cy.get( '.block-editor-block-switcher__toggle' )
				.click()
				.get( '.editor-block-list-item-coblocks-field-textarea' )
				.click();
