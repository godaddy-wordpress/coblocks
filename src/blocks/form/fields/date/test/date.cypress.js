...
describe( 'Test CoBlocks Form Block', function() {
	/**
	 * Test the coblocks form date field transforms.
	 */
	it( 'should transform from coblocks/field-date block to coblocks/field-name, coblocks/field-textarea, coblocks/field-phone, coblocks/field-text, coblocks/field-website, coblocks/field-hidden', function() {
		...
		for ( var index = 0; index < blocksToTransformTo.length; index++ ) {
			...
			// Switch the date field to another type.
			cy.get( '[data-type="coblocks/field-date"]' )
				.click( { force: true } );
			...
			// Check the date field transformed properly.
			...
			cy.get( '.block-editor-block-switcher__toggle' )
				.click()
				.get( '.editor-block-list-item-coblocks-field-date' )
				.click();
