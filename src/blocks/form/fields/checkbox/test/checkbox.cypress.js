...
describe( 'Test CoBlocks Form Block', function() {
	/**
	 * Test the coblocks form checkbox field transforms.
	 */
	it( 'should transform from coblocks/field-checkbox block to coblocks/field-select, coblocks/field-radio', function() {
		...
		cy.get( '[data-type="coblocks/field-checkbox"]' )
			.click( { force: true } );
		...
		for ( var index = 0; index < blocksToTransformTo.length; index++ ) {
			...
			// Switch the checkbox field to another type.
			cy.get( '[data-type="coblocks/field-checkbox"]' )
				.click( { force: true } );
			...
			// Check the checkbox field transformed properly.
